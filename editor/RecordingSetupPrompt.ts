...
export class RecordingSetupPrompt implements Prompt {
	...
	private readonly _keyboardLayout: HTMLSelectElement = select({style: "width: 100%;"},
		...
		option({value: "pianoAtC"}, "Piano starting at C major"),
		option({value: "pianoAtA"}, "Piano starting at A minor"),
		option({value: "pianoTransposingC"}, "Piano transposing C major to song key"),
		option({value: "pianoTransposingA"}, "Piano transposing A minor to song key"),
	);
	...
	public readonly container: HTMLDivElement = div({class: "prompt noSelection recordingSetupPrompt", style: "width: 600px; text-align: right; max-height: 90%;"},
		h2({style: "align-self: center;"},
			"Note Recording Setup"),
		div({style: "display: grid; overflow-y: auto; overflow-x: hidden; flex-shrink: 1;"},
			p("Midbox can record notes as you perform them. You can start recording by pressing Ctrl+Space (or " + EditorConfig.ctrlSymbol + "P)."),
			...
			p("If you have a ", a({href: "https://caniuse.com/midi", target: "_blank"}, "compatible browser"), " on a device connected to a MIDI keyboard, you can use it to perform notes in Midbox! You could also buy other things like a Dubler to hum notes into a microphone while wearing headphones."),
			...
			p("Recorded notes often overlap such that one note ends after the next note already started. In Midbox, these notes get split into multiple notes which may sound different when re-played than they did when you were recording. To fix the sound, you can either manually clean up the notes in the pattern editor, or you could try enabling the \"transition type\" effect on the instrument and setting it to \"continue\"."),
