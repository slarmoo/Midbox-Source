        ...
        if (isNoise) {
            ...
            if (Math.random() < 0.1) {
                ...
                if (instrument.pitchShift != Config.pitchShiftCenter) {
                    ...
                    ])].index, 1, Math.round((Math.random() * 1.35 + 0.35) * 1000) / 1000, false, 0, 1, 4, 0);
                }
            }
            if (Math.random() < 0.1) {
...
export class ChangeEnvelopeAmplitude extends Change {
    constructor(doc: SongDocument, envelopeIndex: number, oldValue: number, newValue: number) {
        super();
        const instrument: Instrument = doc.song.channels[doc.channel].instruments[doc.getCurrentInstrument()];
        if (oldValue != newValue) {
            instrument.envelopes[envelopeIndex].amplitude = newValue;
            instrument.preset = instrument.type;
            doc.notifier.changed();
            this._didSomething();
        }
...
export class ChangeAddEnvelope extends Change {
    constructor(doc: SongDocument) {
        super();
        const instrument: Instrument = doc.song.channels[doc.channel].instruments[doc.getCurrentInstrument()];
        instrument.addEnvelope(0, 0, 0, 1, 1, false, 0, 1, 4, 0);
        instrument.preset = instrument.type;
        doc.notifier.changed();
        this._didSomething();
    }
