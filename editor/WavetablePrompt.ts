...
import { ChangeWavetableCustomWave, ChangeCycleWaves, randomRoundedWave, randomPulses, randomChip, biasedFullyRandom, randomizeWave } from "./changes";
...
export class WavetablePromptCanvas {
	...
	public wavetableIndex: number;

	constructor(doc: SongDocument, wavetableIndex: number, private _curveModeMessage: HTMLElement, private _removeSelectionButton: HTMLElement) {
		this._doc = doc;
	...
	public _randomizeCustomChip = (): void => {
		...
		this._storeChange();
        new ChangeWavetableCustomWave(this._doc, randomGeneratedArray, this.wavetableIndex);
		this.curveModeStep = CurveModeStep.First;
	}
...
export class WavetablePrompt implements Prompt {
	...
	public wavetableCanvas: WavetablePromptCanvas = new WavetablePromptCanvas(this._doc, this._songEditor._wavetableIndices[this._doc.channel][this._doc.getCurrentInstrument()], this.curveModeStepText, this.removeSelectionButton);
	...
	private readonly dragOnSelectionButton1: HTMLButtonElement = button({ style: "width: 90px; position: absolute; top: 152px; align-self: center;", title: "Extend Selection Ends"}, [
	...
	private readonly dragOnSelectionButton2: HTMLButtonElement = button({ style: "width: 90px; position: absolute; top: 152px; align-self: center;", title: "Stretch Items in Selection"}, [
	...
	constructor(private _doc: SongDocument, private _songEditor: SongEditor) {
		...
		this._undoButton.addEventListener("click", this.wavetableCanvas.undo);
		this._redoButton.addEventListener("click", this.wavetableCanvas.redo);
		...
		this._flipHorizontalButton.addEventListener("click", this.wavetableCanvas.flipHorizontally);
		this._flipVerticalButton.addEventListener("click", this.wavetableCanvas.flipVertically);
	...
	private _changeCycleEditorButton = (index: number): void => {
		...
		if (instrument.currentCycle.length < 1) {
	...
	private _removeSelection = (): void => {
		this.wavetableCanvas.clearSelection();
	...
	private _selectStandardDrawType = (): void => {
		...
		this.wavetableCanvas.drawMode = DrawMode.Standard;
		this.wavetableCanvas.clearSelection();
		this.curveModeStepText.style.display = "none";
		this.dragOnSelectionButton1.style.display = "none";
		this.dragOnSelectionButton2.style.display = "none";
	...
	private _selectLineDrawType = (): void => {
		...
		this._drawType_Curve.classList.remove("selected-instrument");
		this._drawType_Selection.classList.remove("selected-instrument");
		this.wavetableCanvas.drawMode = DrawMode.Line;
		this.wavetableCanvas.clearSelection();
		this.curveModeStepText.style.display = "none";
		this.dragOnSelectionButton1.style.display = "none";
		this.dragOnSelectionButton2.style.display = "none";
	...
	private _selectCurveDrawType = (): void => {
		...
		this._drawType_Selection.classList.remove("selected-instrument");
		if (this.wavetableCanvas.drawMode != DrawMode.Curve) this.wavetableCanvas.curveModeStep = CurveModeStep.First;
		this.wavetableCanvas.drawMode = DrawMode.Curve;
		this.wavetableCanvas.clearSelection();
		this.curveModeStepText.style.display = "";
		this.curveModeStepText.textContent = getCurveModeStepMessage(this.wavetableCanvas.curveModeStep);
		this.dragOnSelectionButton1.style.display = "none";
		this.dragOnSelectionButton2.style.display = "none";
	}
	...
	private _selectSelectionDrawType = (): void => {
		...
		this.wavetableCanvas.drawMode = DrawMode.Selection;
		this.wavetableCanvas.clearSelection();
		this.curveModeStepText.style.display = "none";
	...
	public cleanUp = (): void => {
		...
		this._flipHorizontalButton.removeEventListener("click", this.wavetableCanvas.flipHorizontally);
		this._flipVerticalButton.removeEventListener("click", this.wavetableCanvas.flipVertically);
	}

	private _copySettings = (): void => {
		this.wavetableCanvas.copy();
	}
	private _pasteSettings = (): void => {
		this.wavetableCanvas.paste();
	}

	private _extendSelectionPressed = (): void => {
		this.dragOnSelectionButton1.style.display = "none";
		this.dragOnSelectionButton2.style.display = "";
		this.wavetableCanvas.selectionDragEvent = SelectionDragEvent.Stretching;
		this._songEditor.refocusStage();
	}
	private _stretchSelectionPressed = (): void => {
		this.dragOnSelectionButton1.style.display = "";
		this.dragOnSelectionButton2.style.display = "none";
		this.wavetableCanvas.selectionDragEvent = SelectionDragEvent.Extending;
		this._songEditor.refocusStage();
	}
	...
	private _customWavePresetHandler = (event: Event): void => {
        ...
        for (let i: number = 0; i < 64; i++) {
            ...
            this.wavetableCanvas.chipData[i] = customWaveArray[i];
        }

		this.wavetableCanvas._storeChange();
        new ChangeWavetableCustomWave(this._doc, customWaveArray, this.wavetableCanvas.wavetableIndex);
		this.wavetableCanvas.curveModeStep = CurveModeStep.First;
		this.curveModeStepText.textContent = getCurveModeStepMessage(this.wavetableCanvas.curveModeStep);
        this.loadWaveformPresetSelect.selectedIndex = 0;
    }

	private _decideRandomization = (): void => {
		if (this.wavetableCanvas.drawMode != DrawMode.Selection || this.wavetableCanvas.selectionModeStep == SelectionModeStep.NoSelection) {
			this.wavetableCanvas._randomizeCustomChip();
		} else {
			this.wavetableCanvas._randomizeSelection();
		}
		this.curveModeStepText.textContent = getCurveModeStepMessage(this.wavetableCanvas.curveModeStep);
	}
	...
	public whenKeyPressed = (event: KeyboardEvent): void => {
		if ((<Element>event.target).tagName != "BUTTON" && event.keyCode == 13) { // Enter key
			...
			|| event.keyCode == 171 // Some users may have this as +
		  	...
		  	this.wavetableCanvas.shiftSamplesUp();
		  	event.stopPropagation();
		}
		else if (
			event.keyCode == 189 // -
			|| event.keyCode == 173 // Firefox -
		  	...
		  	this.wavetableCanvas.shiftSamplesDown();
		  	event.stopPropagation();
		}
			...
			this.wavetableCanvas.flipHorizontally();
			event.stopPropagation();
		}
		else if (event.keyCode == 78) { // n
			this.wavetableCanvas.flipVertically();
			event.stopPropagation();
		}
			...
			event.stopPropagation();
			if (this.wavetableCanvas.selectionModeStep != SelectionModeStep.NoSelection) {
				this.wavetableCanvas.clearSelection();
			} else {
				this._close();
			}
